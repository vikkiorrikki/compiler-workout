...
let rec eval env conf prg = 
	...
	| instr :: tail -> (
		...
		| (cstack, st, (s, i, o)), CALL f -> eval env ((tail, s)::cstack, st, (s, i, o)) (env#labeled f)
		| (cstack, st, (s, i, o)), BEGIN (arg, xs) -> let rec get_args s arg st = 
			match arg, st with
...
let rec ex_comp expr =
	...
	| Expr.Call (nm, arg) -> List.concat (List.map ex_comp (List.rev arg)) @ [CALL nm]
...
let rec compile' stmt env =
	...
	| Stmt.Call (nm, arg) ->
		List.concat (List.map ex_comp (List.rev arg)) @ [CALL nm], env
	| Stmt.Return expr -> 
		match expr with
		| None -> [END], env
		| Some ex -> ex_comp ex @ [END], env
...
let compile (defs, stmt) = 
	...
	| (name, (arg, locals, body))::defs' ->
		...
		in [LABEL name; BEGIN (arg, locals)] @ c_body @ [END] @ b_defs, env
	| [] -> [], env
